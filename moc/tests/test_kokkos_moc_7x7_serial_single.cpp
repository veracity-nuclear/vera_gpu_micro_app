...
TEST(BasicTest, 7x7_7g_16a_3p_kokkos_single) {
    const char* raw_args[] = {"exe", "data/7x7_7g_16a_3p_serial.h5", "data/c5g7.xsl", "--sweeper", "kokkos", "--device", "serial", "--precision", "single"};
    ...
    Kokkos::initialize(argc, args);
    {
        ...
        std::shared_ptr<BaseMOC> sweeper(new KokkosMOC<Kokkos::Serial, float>(parser));
        EigenSolver solver(parser, sweeper);
        solver.solve();
        EXPECT_NEAR(solver.keff(), 1.340878349, 1.0e-7);
    }
    Kokkos::finalize();
}
