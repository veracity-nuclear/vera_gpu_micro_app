...
struct KokkosLongRay
{
    ...
    public:        // Host views
        ...
        : _radians(radians), _angle_index(angle_index)
        {
            ...
            // Allocate host views
            _h_fsrs = HViewInt1D("fsrs", _nsegs);
            _h_segments = HViewDouble1D("segments", _nsegs);
            ...
            _d_fsrs = Kokkos::create_mirror_view_and_copy(MemorySpace(), _h_fsrs);
        ...
        KOKKOS_INLINE_FUNCTION
        double d_segment(int iseg) const {
            return _d_segments(iseg);
        }
        ...
        KOKKOS_INLINE_FUNCTION
        double segment(int iseg) const {
            return _h_segments(iseg);
        }
