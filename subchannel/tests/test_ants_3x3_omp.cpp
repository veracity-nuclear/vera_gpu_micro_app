...
TEST(SubchannelTest, OpenMPExecution) {
    ...
    for (size_t i = 0; i < N*N; ++i) {
        EXPECT_GT(h_P(i, 0), 0.0) << "Pressure should be positive";
        EXPECT_LT(h_P(i, 0), 1e8) << "Pressure should be reasonable";
    }

    std::cout << "OpenMP execution completed successfully!" << std::endl;
