...
TEST(SubchannelTest, GlobalSurfaceIndex) {
    ...
    EXPECT_EQ(geometry.global_surf_index(0, 0), geometry.boundary);
    EXPECT_EQ(geometry.global_surf_index(0, 1), 0);
    EXPECT_EQ(geometry.global_surf_index(0, 2), geometry.boundary);
    EXPECT_EQ(geometry.global_surf_index(0, 3), 6);
    ...
    EXPECT_EQ(geometry.global_surf_index(1, 3), 7);
    ...
    EXPECT_EQ(geometry.global_surf_index(2, 3), 8);
    ...
    EXPECT_EQ(geometry.global_surf_index(6, 2), 9);
    ...
    EXPECT_EQ(geometry.global_surf_index(7, 2), 10);
...
TEST(SubchannelTest, ToNode_FromNode) {
    ...
    EXPECT_EQ(geometry.surfaces[7].from_node, 1);
    EXPECT_EQ(geometry.surfaces[7].to_node,   4);

    EXPECT_EQ(geometry.surfaces[8].from_node, 2);
    EXPECT_EQ(geometry.surfaces[8].to_node,   5);

    EXPECT_EQ(geometry.surfaces[9].from_node, 3);
    EXPECT_EQ(geometry.surfaces[9].to_node,   6);

    EXPECT_EQ(geometry.surfaces[10].from_node, 4);
    EXPECT_EQ(geometry.surfaces[10].to_node,   7);
