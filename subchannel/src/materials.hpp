...
public:
    ...
    Vector3D Cp(const Vector3D& h) const;

    // viscosity [Pa-s]
    double mu(double h) const;
    Vector3D mu(const Vector3D& h) const;
    ...
    Vector3D k(const Vector3D& h) const;
