...
| `Domain_#####/Solution_Data/fsr_flux` | `float` | (n_fsrs, n_groups) | The flux in each FSR for each energy group. |
...
| `flux` | `float` | (n_cells, n_groups) | The flux in each coarse mesh cell for each energy group. |
| `transport XS` | `float` | (n_cells, n_groups) | The transport cross section in each coarse mesh cell for each energy group. |
| `removal XS` | `float` | (n_cells, n_groups) | The removal cross section in each coarse mesh cell for each energy group. |
| `nu-fission XS` | `float` | (n_cells, n_groups) | The nu-fission cross section in each coarse mesh cell for each energy group. |
| `scattering XS` | `float` | (n_cells, n_groups [to], n_groups [from]) | The scattering cross section in each coarse mesh cell for each energy group. |
| `chi` | `float` | (n_cells, n_groups) | The chi in each coarse mesh cell for each energy group. |
| `surf2cell` | `int` | (n_cells, 2) | The surface to cell mapping for each coarse mesh cell; the first column is the cell index on the positive side (north, east, up) and the second column is the cell index on the negative side (south, west, down). |
| `Dtilde` | `float` | (n_surf, n_groups) | | The `$\tilde{D}$` value for each surface in each energy group. |
| `Dhat` | `float` | (n_surf, n_groups) | The `$\hat{D}$` value for each surface in each energy group. |
